 ...
 * Vercel serverless functions (api/) and the client-side app (src/) —
 * localStorage mode and the monthly-rankings recalculation both run this
 * same logic in the browser. api/_lib/eloCalculator.ts and
 * src/services/eloCalculator.ts are thin re-exports of this module; don't
 * reintroduce a second copy of the logic there.
 *
 * Two real bugs slipped through specifically because those two copies used
 * to drift apart (see git history around calculateGameRatingChanges): a
 * fix landed in one and not the other. Keeping one implementation makes
 * that class of bug structurally impossible.
 *
 * Types here are intentionally minimal and structural (EloPlayer/EloGame)
 * rather than importing api/_lib/types.ts or src/types/index.ts — either
 * choice would make this module depend on one side's type module, which is
 * exactly the asymmetry this refactor is trying to avoid. The real Player/
 * Game types from both sides already satisfy these shapes.
 */

const K_FACTOR = 40;
const STARTING_RATING = 1500;

// Constants para ranking mensual independiente
